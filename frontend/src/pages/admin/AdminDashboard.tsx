  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      {activeTab === 'overview' && (
        <>
          {statsLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">{[1,2,3,4,5,6].map(i => <CardSkeleton key={i} lines={2} />)}</div>
          ) : (
            <>
              ...
              <tbody>
                {farmersData?.farmers?.map((f: any) => (
                  <tr key={f.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-gray-500 whitespace-nowrap">{f.village || '—'}</td>
                    <td className="py-3 px-4 text-gray-500 whitespace-nowrap">{f.user?.email}</td>
                    <td className="py-3 px-4">
